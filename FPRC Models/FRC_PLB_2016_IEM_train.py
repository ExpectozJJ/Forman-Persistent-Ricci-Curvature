...
path1 = "ORC PLB IEM 2016/forman_train_graphs/"
path2 = "ORC PLB IEM 2016/forman_test_graphs/"

if not os.path.exists("ORC PLB IEM 2016"):
    os.mkdir("ORC PLB IEM 2016")
if not os.path.exists("ORC PLB IEM 2016/PQR/"):
    os.mkdir("ORC PLB IEM 2016/PQR/")
if not os.path.exists("ORC PLB IEM 2016/MOL/"):
    os.mkdir("ORC PLB IEM 2016/MOL/")
if not os.path.exists("ORC PLB IEM 2016/Complexes/"):
    os.mkdir("ORC PLB IEM 2016/Complexes/")
...
list2 = np.load("ORC PLB IEM 2016/train_list.npz", allow_pickle=True)["arr_0"]
test_list = np.load("ORC PLB IEM 2016/test_list.npz", allow_pickle=True)["arr_0"]
...
def listprep(name1, name2):
    ...
    np.save("ORC PLB IEM 2016/train_BindingAffinity.npy", train_BindingAffinity)
    np.save("ORC PLB IEM 2016/test_BindingAffinity.npy", test_BindingAffinity)
...
def create_test(threshold, num_of_gaps):
    ...
    test_list = np.load("ORC PLB IEM 2016/test_list.npz", allow_pickle=True)
    for i in range(len(test_list["arr_0"])):
        pdbid = test_list["arr_0"][i]
        print("Processing: ", pdbid)
        data = np.load("ORC PLB IEM 2016/Complexes/"+pdbid+"_complex.npz", allow_pickle=True)
        ...
        np.save("ORC PLB IEM 2016/forman_test_graphs/"+pdbid+"_"+str(threshold)+".npy", X_graphs)
...
def create_train(threshold, num_of_gaps):
    ...
    train_list = np.load("ORC PLB IEM 2016/train_list.npz", allow_pickle=True)
    for i in range(len(train_list["arr_0"])):
        ...
        for i in range(0, len(pro_coords)):
            for j in range(0, len(lig_coords)):
                ...
                X_graphs.append(temp_feature)
        #feature_x.append(X_feature)
        np.save("ORC PLB IEM 2016/forman_train_graphs/"+pdbid+"_"+str(threshold)+".npy", X_graphs)

def gbt_rf(threshold, cutoff):
    X_train = np.load("ORC PLB IEM 2016/train_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    X_test = np.load("ORC PLB IEM 2016/test_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
...
def create_complex(filename):
    pro_data = np.load("ORC PLB IEM 2016/PQR/"+filename+"_pocket.npz", allow_pickle=True)
    lig_data = np.load("ORC PLB IEM 2016/MOL/"+filename+"_ligand.npz", allow_pickle=True)
    ...
    np.savez("ORC PLB IEM 2016/Complexes/"+filename+"_complex.npz", **data)
...
def convertall():
    ...
    for i in range(len(list2)):
        #print(list2[i])
        convertpqr("ORC PLB IEM 2016/PQR/"+list2[i]+"_pocket.pqr")
        convertmol2("ORC PLB IEM 2016/MOL/"+list2[i]+"_ligand.mol2")
        create_complex(list2[i])
    
    for i in range(len(test_list)):
        convertpqr("ORC PLB IEM 2016/PQR/"+test_list[i]+"_pocket.pqr")
        convertmol2("ORC PLB IEM 2016/MOL/"+test_list[i]+"_ligand.mol2")
