...
path1 = "ORC PLB IDM 2016/forman_train_graphs/"
path2 = "ORC PLB IDM 2016/forman_test_graphs/"
...
list2 = np.load("ORC PLB IDM 2016/train_list.npz", allow_pickle=True)["arr_0"]
test_list = np.load("ORC PLB IDM 2016/test_list.npz", allow_pickle=True)["arr_0"]
...
def listprep(name1, name2):
    ...
    np.save("ORC PLB IDM 2016/train_BindingAffinity.npy", train_BindingAffinity)
    np.save("ORC PLB IDM 2016/test_BindingAffinity.npy", test_BindingAffinity)
...
def create_test(threshold, cutoff, num_of_gaps):
    ...
    for i in range(len(test_list["arr_0"])):
        ...
        data = np.load("ORC PLB IDM 2016/Complexes/"+pdbid+"_complex_"+str(cutoff)+".npz", allow_pickle=True)
        ...
        np.save("ORC PLB IDM 2016/forman_test_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)
    ...
    for i in range(len(train_list["arr_0"])):
        ...
        for i in range(0, len(pro_coords)):
            for j in range(0, len(lig_coords)):
                ...
                X_graphs.append(temp_feature)
        #feature_x.append(X_feature)
        np.save("ORC PLB IDM 2016/forman_train_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)

def gbt_rf(threshold, cutoff):
    X_train = np.load("ORC PLB IDM 2016/train_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    X_test = np.load("ORC PLB IDM 2016/test_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    Y_test = np.load("ORC PLB IDM 2016/test_BindingAffinity.npy", allow_pickle=True)
    Y_train = np.load("ORC PLB IDM 2016/train_BindingAffinity.npy", allow_pickle=True)
...
def create_complex(filename, cutoff):
    pro_data = np.load("ORC PLB IDM 2016/PDB/"+filename+"_protein.npz", allow_pickle=True)
    lig_data = np.load("ORC PLB IDM 2016/MOL/"+filename+"_ligand.npz", allow_pickle=True)
    ...
    np.savez("ORC PLB IDM 2016/Complexes/"+filename+"_complex_"+str(cutoff)+".npz", **data)
...
def convertall():
    ...
    for i in range(len(list2)):
        #convertpdb("ORC PLB IDM 2016/PDB/"+refined_set[i]+"_protein.pdb")
        #convertmol2("ORC PLB IDM 2016/MOL/"+refined_set[i]+"_ligand.mol2")
