...
for i in range(len(all_comb)):
    ...
    for pdbid in train_list_2007["arr_0"]:
        ...
        for c in np.linspace(0, 15, 150):
            G = gen_lignet(all_comb[i], pdbid, c, path1)
        ...
        nx.write_gpickle([orc_temp, frc_temp], path1+"lig_IDM_graphs/"+pdbid+"_15_"+comb_d[i]+"_ligand.gpickle")

    for pdbid in test_list_2007["arr_0"]:
        print("Processing v2007: ", pdbid)
        orc_temp = []
        frc_temp = []
        for c in np.linspace(0, 15, 150):
...
for pdbid in train_list_2013["arr_0"]:
    ...
    np.save(path2+"lig_IDM_graphs/"+pdbid+"_ollivier_15_C_ligand.npy", orc_temp)
    np.save(path2+"lig_IDM_graphs/"+pdbid+"_forman_15_C_ligand.npy", frc_temp)
