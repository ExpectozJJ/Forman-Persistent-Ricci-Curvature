...
for i in range(len(all_comb)):
    ...
    for pdbid in train_list_2016["arr_0"]:
        ...
        frc_temp = []
        for c in np.linspace(0, 15, 150):
            G = gen_lignet(all_comb[i], pdbid, c, path3)
            if G.number_of_edges() > 0:
                orc = OllivierRicci(G, alpha=0.5)
                orc.compute_ricci_curvature()
                frc = FormanRicci(G)
        ...
        nx.write_gpickle([orc_temp, frc_temp], path3+"lig_IDM_graphs/"+pdbid+"_15_"+comb_d[i]+"_ligand.gpickle")

    for pdbid in test_list_2016["arr_0"]:
        print("Processing v2016: ", pdbid)
        orc_temp = []
        frc_temp = []
        for c in np.linspace(0, 15, 150):
