...
cnt = 0
for t in test_list:
    temp = np.load("ORC PLB IDM 2016/forman_test_graphs/"+t+"_10_15.npy", allow_pickle=True)
    ...
    cnt += 1
np.save("ORC PLB IDM 2016/forman_test_set_"+str(fp)+"_model_4.npy", test_set)

# Build Training Set for Molecular Descriptors 
train_list = np.load("ORC PLB IDM 2016/train_list.npz", allow_pickle=True)["arr_0"]
train_set = []
cnt = 0
for t in train_list:
    temp = np.load("ORC PLB IDM 2016/forman_train_graphs/"+t+"_10_15.npy", allow_pickle=True)
    print("Processing: ", t, cnt)
    train_features = []
    for i in range(len(temp)):
        temp_feature = []
    ...
    cnt += 1
np.save("ORC PLB IDM 2016/forman_train_set_"+str(fp)+"_model_4.npy", train_set)

X_train = np.load("ORC PLB IDM 2016/forman_train_set_"+str(fp)+"_model_4.npy", allow_pickle=True)
X_test = np.load("ORC PLB IDM 2016/forman_test_set_"+str(fp)+"_model_4.npy", allow_pickle=True)
...
X_test = np.reshape(X_test, (len(X_test), 36*10*fp*20))
X_train = np.reshape(X_train, (len(X_train), 36*10*fp*20))
...
np.save("ORC PLB IDM 2016/forman_results_"+str(fp)+"_model_4.npy", [results, gbt_values, rf_values])