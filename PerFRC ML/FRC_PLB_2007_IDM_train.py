...
path1 = "ORC PLB IDM 2007/forman_train_graphs/"
path2 = "ORC PLB IDM 2007/forman_test_graphs/"
...
list2 = np.load("ORC PLB IDM 2007/train_list.npz", allow_pickle=True)["arr_0"]
...
def listprep(name1, name2):
    ...
    np.save("ORC PLB IDM 2007/train_BindingAffinity.npy", train_BindingAffinity)
    np.save("ORC PLB IDM 2007/test_BindingAffinity.npy", test_BindingAffinity)
...
def create_test(threshold, cutoff, num_of_gaps):
    test_list = np.load("ORC PLB IDM 2007/test_list.npz", allow_pickle=True)
    for i in range(a, b+1):
        pdbid = test_list["arr_0"][i]
        print("Processing: ", pdbid)
        data = np.load("ORC PLB IDM 2007/Complexes/"+pdbid+"_complex_"+str(cutoff)+".npz", allow_pickle=True)
        ...
        np.save("ORC PLB IDM 2007/forman_test_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)
...
def create_train(threshold, cutoff, num_of_gaps):
    ...
    train_list = np.load("ORC PLB IDM 2007/train_list.npz", allow_pickle=True)
    for i in range(a, b+1):
        ...
        for i in range(0, len(pro_coords)):
            for j in range(0, len(lig_coords)):
                ...
                X_graphs.append(temp_feature)
        #feature_x.append(X_feature)
        np.save("ORC PLB IDM 2007/forman_train_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)

def gbt_rf(threshold, cutoff):
    X_train = np.load("ORC PLB IDM 2007/train_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    X_test = np.load("ORC PLB IDM 2007/test_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    Y_test = np.load("ORC PLB IDM 2007/test_BindingAffinity.npy", allow_pickle=True)
    Y_train = np.load("ORC PLB IDM 2007/train_BindingAffinity.npy", allow_pickle=True)
...
def create_complex(filename, cutoff):
    pro_data = np.load("ORC PLB IDM 2007/PDB/"+filename+"_protein.npz", allow_pickle=True)
    lig_data = np.load("ORC PLB IDM 2007/MOL/"+filename+"_ligand.npz", allow_pickle=True)
    ...
    np.savez("ORC PLB IDM 2007/Complexes/"+filename+"_complex_"+str(cutoff)+".npz", **data)
...
def convertall():
    ...
    for i in range(len(list2)):
        #convertpdb("ORC PLB IDM 2007/PDB/"+refined_set[i]+"_protein.pdb")
        #convertmol2("ORC PLB IDM 2007/MOL/"+refined_set[i]+"_ligand.mol2")
