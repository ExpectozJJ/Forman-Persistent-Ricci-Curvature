...
cnt = 0
for t in test_list:
    temp = np.load("ORC PLB IEM 2007/forman_test_graphs/"+t+"_1.npy", allow_pickle=True)
    ...
    cnt += 1
np.save("ORC PLB IEM 2007/forman_test_set_"+str(fp)+"_model_4.npy", test_set)

# Build Training Set for Molecular Descriptors 
train_list = np.load("ORC PLB IEM 2007/train_list.npz", allow_pickle=True)["arr_0"]
train_set = []
cnt = 0
for t in train_list:
    temp = np.load("ORC PLB IEM 2007/forman_train_graphs/"+t+"_1.npy", allow_pickle=True)
    print("Processing: ", t, cnt)
    train_features = []
    for i in range(len(temp)):
        temp_feature = []
    ...
    cnt += 1
np.save("ORC PLB IEM 2007/forman_train_set_"+str(fp)+"_model_4.npy", train_set)
"""

X_train = np.load("ORC PLB IEM 2007/forman_train_set_"+str(fp)+"_model_4.npy", allow_pickle=True)
X_test = np.load("ORC PLB IEM 2007/forman_test_set_"+str(fp)+"_model_4.npy", allow_pickle=True)
...
np.save("ORC PLB IEM 2007/forman_results_"+str(fp)+"_model_4.npy", [results, gbt_values, rf_values])